...
boolean WiFiManager::autoConnect() {
  ...
  String ssid = "MyGasValve" + String(ESP.getChipId());
...
Adafruit_SSD1306 display = Adafruit_SSD1306(128, 32, &Wire);
 ...
 // return startConfigPortal(apName, apPassword);
  ...
  display.print("Connected to Hotspot");
  display.setCursor(0, 8);
  display.print("MyGasValue then ");
  display.setCursor(0, 16);
  display.print("goto 192.168.4.1 ");
  display.setCursor(0, 24);
  display.print("in your webbrowser");
  display.display(); // actually display all of the above
 return startConfigPortal("MyGasValve", apPassword);
}
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  String page = FPSTR(HTTP_HEADER);
  page.replace("{v}", "Config MyGasValve");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
